...
\tikzstyle{process} = [rectangle, minimum width=3.2cm, minimum height=1cm, text centered, draw=black, fill=blue!10]
...
The system consists of a decoupled client-server architecture. Figure~\ref{fig:architecture} demonstrates the operational flow between the browser client, API middleware, and cloud database.

\begin{figure}[h!]
\centering
\begin{tikzpicture}[node distance=1.8cm, auto]
    % Nodes
    \node (client) [process, fill=blue!15] {\textbf{React 19 Client} \\ (face-api.js / WebGL)};
    \node (trpc) [process, right=1.5cm of client, fill=green!15] {\textbf{tRPC Middleware} \\ (Express / Zod)};
    \node (db) [process, right=1.5cm of trpc, fill=orange!15] {\textbf{Aiven MySQL 8.0} \\ (Drizzle ORM)};
    
    % Connections
    \draw[arrow] (client) -- node[above] {\small Biometric Descriptor} node[below] {\small (JSON Payload)} (trpc);
    \draw[arrow] (trpc) -- node[above] {\small SQL Query} node[below] {\small (TLS 1.3)} (db);
    \draw[arrow] (db) edge[bend left=30] node[below] {\small Matched Record} (trpc);
    \draw[arrow] (trpc) edge[bend left=30] node[below] {\small Session Cookie / Auth Result} (client);
\end{tikzpicture}
\caption{End-to-End Component Architecture Diagram of FaceLiveness AI.}
...
Figure~\ref{fig:landmarks} illustrates the 68-point landmark geometry used for liveness extraction.

\begin{figure}[h!]
\centering
\begin{tikzpicture}[scale=0.8]
    % Face Outline
    \draw[thick, fill=blue!5] (0,0) ellipse (2.5cm and 3.2cm);
    
    % Eyes
    \draw[fill=black] (-1,0.8) circle (0.15cm) node[above=0.2cm] {\tiny $P_{36}-P_{41}$};
    \draw[fill=black] (1,0.8) circle (0.15cm) node[above=0.2cm] {\tiny $P_{42}-P_{47}$};
    
    % Nose
    \draw[thick, fill=red!20] (0,0.2) -- (-0.3,-0.6) -- (0.3,-0.6) -- cycle;
    \node at (0,-0.8) {\tiny Nose Tip $P_{27}$};
    
    % Mouth
    \draw[thick, fill=red!30] (-0.8,-1.6) quadratic bend (0,-1.3) .. (0.8,-1.6) quadratic bend (0,-2.0) .. (-0.8,-1.6);
    \node at (0,-2.3) {\tiny Lips $P_{48}-P_{67}$};
\end{tikzpicture}
\caption{68 Facial Landmark Points Mapping and Regional Geometry.}
\label{fig:landmarks}
\end{figure}
...
Figure~\ref{fig:flowchart} shows the dual-stage liveness state machine flow.

\begin{figure}[h!]
\centering
\begin{tikzpicture}[node distance=1.5cm]
    \node (start) [startstop] {Start Camera Capture};
    \node (detect) [process, below of=start] {Detect Face \& 68 Landmarks};
    \node (blink) [decision, below of=detect, yshift=-0.5cm] {EAR Blink Detected?};
    \node (challenge) [process, right of=blink, xshift=3cm] {Secondary Challenge (Head Turn/Mouth)};
    \node (chalcheck) [decision, below of=challenge, yshift=-0.5cm] {Challenge Passed?};
    \node (extract) [process, left of=chalcheck, xshift=-3cm] {Extract 128-D Embedding Vector};
    \node (match) [decision, below of=extract, yshift=-0.5cm] {Euclidean Distance $< 0.60$?};
    \node (success) [startstop, fill=green!25, right of=match, xshift=3cm] {AUTHENTICATED};
    \node (fail) [startstop, fill=red!25, left of=match, xshift=-2.5cm] {REJECTED};

    \draw[arrow] (start) -- (detect);
    \draw[arrow] (detect) -- (blink);
    \draw[arrow] (blink) -- node[above] {Yes} (challenge);
    \draw[arrow] (blink) -- node[left] {No} (detect);
    \draw[arrow] (challenge) -- (chalcheck);
    \draw[arrow] (chalcheck) -- node[left] {Yes} (extract);
    \draw[arrow] (chalcheck) edge[bend right=45] node[right] {No} (fail);
    \draw[arrow] (extract) -- (match);
    \draw[arrow] (match) -- node[above] {Yes} (success);
    \draw[arrow] (match) -- node[above] {No} (fail);
\end{tikzpicture}
\caption{Dual-Stage Interactive Liveness Verification Flowchart.}
\label{fig:flowchart}
\end{figure}
