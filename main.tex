...
% --- Table Rule Width Fix for PDF Viewers ---
...
{\Large \textbf{FACELIVENESS AI: A FACE LIVENESS DETECTION SYSTEM}}\\[0.8cm]
...
{\Large \textbf{B.Tech}}\\[0.6cm]
{\large by}\\[0.1cm]
{\large \textbf{Vikash Kumar} (\textbf{23AI054})}\\[0.2cm]
{\large \textit{to the}}\\[0.1cm]

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.3\textwidth]{gsv.jpg}
  \label{fig:my_image}
\end{figure}
...
\begin{doublespace}
I take this opportunity to express my sincere thanks to my mentor \textbf{Sh. Pankaj Roy} for their invaluable guidance and support throughout the \textbf{8-week} duration of this summer internship project. I would also like to express my gratitude towards them for showing confidence in me. It was a privilege to have a great experience working under them in a cordial and inspiring technical environment.

I am very much thankful to \textbf{National Capital Region Transport Corporation (NCRTC), Delhi} for providing me the opportunity of pursuing this summer internship project: \textbf{FaceLiveness AI: A Face Liveness Detection System}. I am very much thankful to the project supervisors and Industry mentors for providing computing resources and technical guidance required to execute this work.
...
\hline
\rowcolor{gray!15} \textbf{Symbol / Abbr.} & \textbf{Description} \\ \hline
$\text{EAR}$ & Eye Aspect Ratio \\ \hline
$\text{FLD}$ & Face Liveness Detection \\ \hline
$\text{PAD}$ & Presentation Attack Detection \\ \hline
$\text{FAR}$ & False Acceptance Rate \\ \hline
$\text{FRR}$ & False Rejection Rate \\ \hline
$\mathbb{R}^{128}$ & 128-Dimensional Real-Valued Vector Space \\ \hline
$\text{tRPC}$ & TypeScript Remote Procedure Call \\ \hline
$\text{ORM}$ & Object-Relational Mapping \\ \hline
$\text{PaaS}$ & Platform as a Service \\ \hline
$\text{SSL/TLS}$ & Secure Sockets Layer / Transport Layer Security \\ \hline
$\text{FPS}$ & Frames Per Second \\ \hline
$\text{MobileNetV2}$ & Mobile Neural Network Architecture Version 2 \\ \hline
\end{tabular}
\end{table}
...
\begin{enumerate}
    ...
    \item Achieve 6 to 7 second verification times with $> 98\%$ recognition accuracy and zero server GPU hosting costs.
    \item Deploy the full-stack system completely free of cost on public cloud PaaS infrastructure with automated HTTPS and SSL encryption.
\end{enumerate}

\subsection{Organization of Report}
The remainder of this report is organized as follows:
\begin{itemize}
    \item \textbf{Methodology:} Details the full system architecture, deep learning model pipeline, mathematical liveness formulas, and database schema.
    \item \textbf{Exploratory Data Analysis \& Benchmarks:} Presents accuracy performance evaluation, FAR/FRR trade-off curves, and latency measurements.
    \item \textbf{Outcomes \& Discussion:} Provides an in-depth comparative technology trade-off analysis and threat matrix evaluation.
    \item \textbf{Implications \& Policy Recommendations:} Discusses GDPR/CCPA data privacy compliance and production security policies.
    \item \textbf{Weekly Overview of Internship Activity:} Outlines the week-by-week progress across the 8-week internship period.
\end{itemize}

\subsection{Organization Information}
This project was carried out as part of the summer internship requirements at \textbf{National Capital Region Transport Corporation (NCRTC)} – a joint venture company of Govt of India and States of Delhi, Haryana, Rajasthan and Uttar Pradesh is mandated for implementing the Namo Bharat project across the National Capital Region, ensuring a balanced and sustainable urban development through better connectivity and access.
...
\subsubsection{Facial Feature Detection and Landmark Extraction}
...
\subsubsection{Eye Aspect Ratio (EAR) Blink Detection}
...
\subsubsection{Head Yaw Rotation Ratio}
...
\subsubsection{1:1 Euclidean Descriptor Distance}
...
\begin{tabular}{|p{6.5cm}|p{4.2cm}|p{4.2cm}|}
\hline
\rowcolor{gray!15} \textbf{Metric Parameter} & \textbf{Measured Value} & \textbf{Target Benchmark} \\ \hline
...
\small
\begin{tabular}{|p{2.5cm}|p{3.0cm}|p{4.5cm}|p{4.8cm}|}
\hline
\rowcolor{gray!15} \textbf{Component} & \textbf{Selected Tech} & \textbf{Key Advantages} & \textbf{Disadvantages / Mitigations} \\ \hline
\textbf{Frontend} & React 19 + Vite & Virtual DOM, sub-second HMR builds. & Larger bundle size; mitigated by code-splitting. \\ \hline
\textbf{Biometric Engine} & face-api.js & Zero server GPU cost, client privacy. & Dependent on client CPU/GPU. \\ \hline
...
\small
\begin{tabular}{|p{3.5cm}|p{5.5cm}|p{6.2cm}|}
\hline
\rowcolor{gray!15} \textbf{Attack Vector} & \textbf{Threat Mechanism} & \textbf{System Mitigation Control} \\ \hline
...
\small
\begin{tabular}{|p{1.5cm}|p{4.2cm}|p{9.0cm}|}
\hline
\rowcolor{gray!15} \textbf{Week} & \textbf{Focus Area} & \textbf{Key Technical Activities \& Achievements} \\ \hline
\textbf{Week 1} & Literature Review \& Requirements Analysis & Researched presentation attack detection (PAD) standards, surveyed dlib/TensorFlow.js algorithms, defined project scope and target metrics. \\ \hline
\textbf{Week 2} & Computer Vision Engine Setup & Integrated \texttt{face-api.js} with MobileNetV2 TinyFaceDetector, implemented camera stream viewport rendering in React 19. \\ \hline
\textbf{Week 3} & Liveness Algorithm Implementation & Formulated Eye Aspect Ratio (EAR) blink detection state machine, implemented head yaw and mouth ratio verification modules. \\ \hline
\textbf{Week 4} & Full-Stack API Architecture & Built tRPC API router with Zod input validation schemas, setup Express server middleware, configured JWT authentication. \\ \hline
\textbf{Week 5} & Database \& Vector Embeddings & Provisioned Aiven Cloud MySQL 8.0 instance, implemented Drizzle ORM schemas, built 1:1 Euclidean vector matching procedure. \\ \hline
\textbf{Week 6} & Security Guardrails \& Spoof Testing & Integrated multi-face detection guard, implemented bcrypt password hashing, performed anti-spoofing tests with printed photos/video replays. \\ \hline
\textbf{Week 7} & Dockerization \& Cloud Deployment & Created multi-stage Alpine Dockerfile, configured Render PaaS automated GitHub CI/CD build pipeline with SSL certificate enforcement. \\ \hline
\textbf{Week 8} & Performance Benchmarking \& Report & Conducted 100 benchmark test authentication runs, compiled FAR/FRR accuracy metrics, finalized comprehensive technical documentation. \\ \hline
\end{tabular}
\end{table}
